...
Class Master extends DBConnection {
	...
	function save_withdrawals1(){
		...
		if(empty($id)){
			//sql bind parameters
			$sql = $this->conn->prepare("INSERT into withdrawals set active='N',date_created= ?, user_id= ?, amount= ?, description= ?,agent_id=?");
			$sql->bind_param("sidsi",  $_POST['date_created'],$_POST['user_id'], $_POST['amount'], $_POST['description'], $_POST['agent_id']);
			$sql->execute();
